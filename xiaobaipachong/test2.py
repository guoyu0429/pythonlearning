...
#思路是先找一个发布代理IP的网站（百度一下很多的！）从这个网站爬取出代理IP 用来访问网页；当本地IP失效时，开始使用代理IP，代理IP失败六次后取消代理IP

class download:
    
    def __init__(self):
        self.iplist=[]#初始化一个list用来存放我们获取到的IP
        html=requests.get("http://haoip.cc/tiqu.htm")#获得代理IP的网站
        iplistn=re.findall(r'r/>(.*?)<b',html.text,re.S)##表示从html.text中获取所有r/><b中的内容，re.S的意思是包括匹配包括换行符，findall返回的是个list哦！
        for ip in iplistn:
            i=re.sub('\n','',ip)#re.sub 是re模块替换的方法，这儿表示将\n替换为空
            self.iplist.append(i.strip())#添加到我们上面初始化的list里面, i.strip()的意思是去掉字符串的空格哦！
